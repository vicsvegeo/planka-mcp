...
export async function getBoardSummary(params: GetBoardSummaryParams) {
  ...
  try {
    ...
    const backlogList = listsWithCards.find(
      (list: any) => list.name?.toLowerCase() === "backlog",
    );
    const inProgressList = listsWithCards.find(
      (list: any) => list.name?.toLowerCase() === "in progress",
    );
    const testingList = listsWithCards.find(
      (list: any) => list.name?.toLowerCase() === "testing",
    );
    const doneList = listsWithCards.find(
      (list: any) => list.name?.toLowerCase() === "done",
    );
    ...
    const urgentCards = listsWithCards
      ...
      .filter((card: any) =>
        card.labelIds?.some((labelId: string) =>
          boardLabels.find(
            (label: any) =>
              label.id === labelId && label.name?.toLowerCase() === "urgent",
          ),
        ),
      ).length;
    ...
    const bugCards = listsWithCards
      ...
      .filter((card: any) =>
        card.labelIds?.some((labelId: string) =>
          boardLabels.find(
            (label: any) =>
              label.id === labelId && label.name?.toLowerCase() === "bug",
          ),
        ),
      ).length;
